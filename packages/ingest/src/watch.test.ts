...
function toolUseLine(uuid: string, parentUuid: string, sid: string, cwd: string, ts: string, toolId: string, name: string, cmd: string): string {
  return line({
    type: "assistant",
    uuid,
    parentUuid,
    ...
    cwd,
    timestamp: ts,
    message: { role: "assistant", content: [{ type: "tool_use", id: toolId, name, input: { command: cmd } }] },
  });
}

function toolResultLine(uuid: string, parentUuid: string, sid: string, cwd: string, ts: string, toolId: string, stdout: string): string {
  return line({
    type: "user",
    uuid,
    parentUuid,
    ...
    message: { role: "user", content: [{ type: "tool_result", tool_use_id: toolId, is_error: false, content: stdout }] },
  });
}

function textLine(uuid: string, parentUuid: string, sid: string, cwd: string, ts: string, text: string): string {
  return line({
    type: "assistant",
    uuid,
    parentUuid,
...
test("watch: catch-up, live append, new file, idempotent restart", async () => {
  ...
  writeFileSync(
    ...
    toolUseLine("w2", "w1", SID_W1, "/home/x/watch-proj", "2025-01-01T00:00:01.000Z", "t1", "Bash", "echo hi") +
    toolResultLine("w3", "w2", SID_W1, "/home/x/watch-proj", "2025-01-01T00:00:02.000Z", "t1", "hi"),
  );
  ...
  appendFileSync(
    sessionFile1,
    toolUseLine("w4", "w3", SID_W1, "/home/x/watch-proj", "2025-01-01T00:01:00.000Z", "t2", "Bash", "ls"),
  );
  ...
  await runIngest({
    ...
    root,
    db,
    onProgress: (n) => { insertedOnRestart = n; },
  });
