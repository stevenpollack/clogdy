...
test("add + flush inserts rows and maps columns", () => {
  ...
  const tu = db.query("SELECT tool, command, corr, kind FROM event WHERE uuid='a' AND block_idx=0").get() as any;
  expect(tu).toEqual({ tool: "Bash", command: "ls", corr: "t1", kind: "tool_use" });

  const tr = db.query("SELECT is_error, corr FROM event WHERE uuid='a' AND block_idx=1").get() as any;
...
test("setCursor upserts", () => {
  ...
  let row = db.query("SELECT offset, inode FROM ingest_cursor WHERE path='/p/a.jsonl'").get() as any;
...
test("upsertSession keeps max last_ts and min first_ts", () => {
  const w = makeWriter(db);
  w.upsertSession({ sessionId: "s1", project: "p", cwd: "/c", path: "/f", ts: 500, gitBranch: "main" });
  w.upsertSession({ sessionId: "s1", project: "p", cwd: null, path: "/f", ts: 1500, gitBranch: null });
  w.upsertSession({ sessionId: "s1", project: "p", cwd: null, path: "/f", ts: 200, gitBranch: null });
  const row = db.query("SELECT first_ts, last_ts, cwd, git_branch FROM session WHERE session_id='s1'").get() as any;
