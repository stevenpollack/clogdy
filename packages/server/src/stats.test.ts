...
/** Build one JSONL transcript line in the real transcript shape (mirrors duck.test.ts). */
function line(o: {
  uuid: string;
  ts: number;
  content: unknown[];
...
describe("/api/stats proxy → analytics CLI", () => {
  it("toolCounts: 200 and matches a direct CLI spawn", async () => {
    ...
    const spawned = Bun.spawnSync(
      ["bun", "run", "v2:analytics", "--", "--db", dbPath, "--metric", "toolCounts", "--filters", "{}"],
      { cwd: REPO_ROOT },
    );
    ...
    const asMap = (d: any) =>
      Object.fromEntries((d.data as Array<{ tool: string; count: number }>).map((r) => [r.tool, r.count]));
