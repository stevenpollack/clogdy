...
/** Build one JSONL transcript line (real Claude shape). */
function line(o: {
  uuid: string;
  ts: number;
  content: unknown[];
...
function buildTranscriptA(cwdA: string): string {
  const lines = [
    // Bash #1
    line({ uuid: "a-bash1-use", ts: T + 0, sessionId: SESSION_A, cwd: cwdA,
      content: [{ type: "tool_use", id: "ta-b1", name: "Bash", input: { command: "ls" } }] }),
    line({ uuid: "a-bash1-res", ts: T + 100, role: "user", sessionId: SESSION_A, cwd: cwdA,
      content: [{ type: "tool_result", tool_use_id: "ta-b1", content: "file1\nfile2", is_error: false }],
      toolUseResult: { stdout: "file1\nfile2", stderr: "" } }),
    // Bash #2
    line({ uuid: "a-bash2-use", ts: T + 200, sessionId: SESSION_A, cwd: cwdA,
      content: [{ type: "tool_use", id: "ta-b2", name: "Bash", input: { command: "pwd" } }] }),
    line({ uuid: "a-bash2-res", ts: T + 300, role: "user", sessionId: SESSION_A, cwd: cwdA,
      content: [{ type: "tool_result", tool_use_id: "ta-b2", content: "/home/user", is_error: false }],
      toolUseResult: { stdout: "/home/user", stderr: "" } }),
    // Bash #3 — ERROR result (the required error event)
    line({ uuid: "a-bash3-use", ts: T + 400, sessionId: SESSION_A, cwd: cwdA,
      content: [{ type: "tool_use", id: "ta-b3", name: "Bash", input: { command: "cat /missing" } }] }),
    line({ uuid: "a-bash3-res", ts: T + 500, role: "user", sessionId: SESSION_A, cwd: cwdA,
      content: [{ type: "tool_result", tool_use_id: "ta-b3", content: "No such file", is_error: true }],
      toolUseResult: { stdout: "", stderr: "cat: /missing: No such file or directory" } }),
    // Read #1
    line({ uuid: "a-read1-use", ts: T + 600, sessionId: SESSION_A, cwd: cwdA,
      content: [{ type: "tool_use", id: "ta-r1", name: "Read", input: { file_path: "/tmp/readme.txt" } }] }),
    line({ uuid: "a-read1-res", ts: T + 700, role: "user", sessionId: SESSION_A, cwd: cwdA,
      content: [{ type: "tool_result", tool_use_id: "ta-r1", content: "Hello world", is_error: false }],
...
function buildTranscriptB(cwdB: string): string {
  const lines = [
    // Edit #1
    line({ uuid: "b-edit1-use", ts: T + 1000, sessionId: SESSION_B, cwd: cwdB,
      content: [{ type: "tool_use", id: "tb-e1", name: "Edit",
        input: { file_path: "/tmp/main.ts", old_string: "foo", new_string: "bar" } }] }),
    line({ uuid: "b-edit1-res", ts: T + 1100, role: "user", sessionId: SESSION_B, cwd: cwdB,
      content: [{ type: "tool_result", tool_use_id: "tb-e1", content: "saved", is_error: false }],
      toolUseResult: { stdout: "saved", stderr: "" } }),
    // Edit #2
    line({ uuid: "b-edit2-use", ts: T + 1200, sessionId: SESSION_B, cwd: cwdB,
      content: [{ type: "tool_use", id: "tb-e2", name: "Edit",
        input: { file_path: "/tmp/README.md", old_string: "teh", new_string: "the" } }] }),
    line({ uuid: "b-edit2-res", ts: T + 1300, role: "user", sessionId: SESSION_B, cwd: cwdB,
...
beforeAll(() => {
  ...
  const res = Bun.spawnSync(
    ["bun", "run", "v2:ingest", "--backfill", "--root", tree, "--db", dbPath],
    { cwd: REPO_ROOT },
  );
  if (res.exitCode !== 0) {
    throw new Error(
...
describe("e2e POST /api/query", () => {
  ...
  describe("(a) facet scoping", () => {
    ...
    it(
      "filter{project:'projA'} → {Bash:3, Read:1} — Edit absent (projB-only)",
      async () => {
    ...
    it(
      "filter{project:'projB'} → {Edit:2} — Bash and Read absent (projA-only)",
      async () => {
  ...
  describe("(b.2) SQL guard rejections → 400", () => {
    ...
    it(
      "comment-smuggled multi-statement SELECT 1 /* */; DROP TABLE event → 400",
      async () => {
