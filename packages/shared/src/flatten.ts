...
export function flattenLine(
  ...
  const parentUuid: string | null =
    typeof line.parentUuid === "string" ? line.parentUuid : null;
  const gitBranch: string | null =
    typeof line.gitBranch === "string" ? line.gitBranch : null;
  const uuid: string =
    ...
    if (type === "tool_use") {
      ...
      if (tur) {
        if (Array.isArray(tur.structuredPatch)) {
          ...
          const size =
            tur.bytes < 1024 ? `${tur.bytes}B` : `${(tur.bytes / 1024).toFixed(1)}KB`;
          const dur =
      ...
      out.push(
        mk("text", blockIdx, { text: typeof block.text === "string" ? block.text : null }),
      );
