...
describe("flattenLine drop rules", () => {
  ...
  it("line without message → [] (file-history-snapshot shape)", () => {
    expect(
      flattenLine(j({ type: "file-history-snapshot", uuid: "x", messageId: "m" }), 0),
    ).toEqual([]);
  });
  it("null JSON → []", () => {
...
describe("isError semantics", () => {
  it("tool_result is_error:true → isError:true", () => {
    const ev = flattenLine(
      j({
        uuid: "u",
        message: { content: [{ type: "tool_result", tool_use_id: "t", content: "x", is_error: true }] },
      }),
      0,
    );
  ...
  it("text block → isError:null", () => {
    const ev = flattenLine(j({ uuid: "u", message: { content: [{ type: "text", text: "x" }] } }), 0);
