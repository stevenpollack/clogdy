...
const TRANSCRIPT_X = [
  ...
  line({ uuid: "x2", ts: T, sessionId: SESSION_X, cwd: CWD_X,
  ...
  line({ uuid: "x4", ts: T + 100, role: "user", sessionId: SESSION_X, cwd: CWD_X,
...
const TRANSCRIPT_Y = [
  ...
  line({ uuid: "y2", ts: T + 300, role: "user", sessionId: SESSION_Y, cwd: CWD_Y,
...
beforeAll(() => {
  ...
  const res = Bun.spawnSync(
    ["bun", "run", "v2:ingest", "--backfill", "--root", tree, "--db", tmpDb],
    { cwd: REPO_ROOT },
  );
  if (res.exitCode !== 0) {
    throw new Error(
...
describe("runQuery via withDuck", () => {
  ...
  it("(a) GROUP BY tool: columns=[tool,n], tool_use counts across all projects", async () => {
    ...
    const byTool = Object.fromEntries(
      result.rows.map((r) => [r[0] as string, Number(r[1])]),
    );
